...
class _FlameTask:
    ...
    def _load_from_full_task_file(self):
        ...
        try:
            logger.debug(f'Loading full task data for {self.get_uuid()}')
    ...
    def get_fields(self, field_names):
        if set(field_names).isdisjoint(self._modelled.unloadable_field_names()):
            # TODO: duplicate state and uuid in always_loaded_task_data just to avoid lock?
...
def select_from_task(select_paths, select_descendants, task: _FlameTask, task_graph: FlameTaskGraph):
    selected_dict = {}
    # FIXME: as_dict may load here even when the paths don't need it to!
    paths_update_dict = _jsonpath_get_paths(select_paths, task.as_dict())
